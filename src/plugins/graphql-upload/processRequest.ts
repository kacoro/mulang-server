...
export interface ProcessRequestFunction  {
...
export const processRequest:ProcessRequestFunction = function(
  request,
  response,
  {
  ...
  return new Promise((resolve, reject) => {
     ...
     * >}
     */
    let operations: GraphQLOperation | GraphQLOperation[];
     ...
     * >}
     */
    let operationsPath: objectPath.ObjectPathBound<any>;
     ...
     */
    function exit(error: createError.HttpError, isParserError = false) {
      if (exitError) return;
    ...
    parser.on("field", (fieldName: any, value: string, { valueTruncated }: any) => {
      if (valueTruncated)
        return exit(
          createError(
            413,
    ...
    parser.on(
      "file",
      (fieldName: any, stream: { unpipe?: any; resume: any; on: any; pipe?: any; }, { filename, encoding, mimeType: mimetype }: any) => {
        if (!map) {
          ignoreStream(stream);
          return exit(
            createError(
        ...
        const file = {
          ...
          mimetype,
          encoding,
          createReadStream(options: string | undefined) {
    ...
    parser.on("error", (/** @type {Error} */ error: any) => {
      exit(error, true);
    });
